...
- MCP sidecar package [`mcp/`](mcp/) (`kms-secp256k1-api-mcp`): Make/Docker lifecycle + HTTP API tools (stdio / Streamable HTTP on `:9789`); Docker image published to Hub + GHCR (`make mcp-docker-push-*`); transport + create/delete tests; see [`docs/mcp.md`](docs/mcp.md)
