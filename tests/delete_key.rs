...
mod tests {
    ...
    async fn test_delete_key_returns_200_when_key_exists() {
        ...
        let url = format!(
            "http://127.0.0.1:4000/deleteKey?public_key={}",
            created.public_key
        );
    ...
    async fn test_delete_key_returns_200_when_key_does_not_exist() {
        ...
        let url = format!("http://127.0.0.1:4000/deleteKey?public_key={fake_key}");
        let resp = client
            .delete(&url)
            .send()
            .await
    ...
    async fn test_delete_key_returns_404_when_disabled() {
        ...
        let url = "http://127.0.0.1:4000/deleteKey?public_key=dummykey";
        let resp = client
            .delete(url)
            .send()
            .await
